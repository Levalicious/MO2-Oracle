...
class PredictPlugin(IPluginTool):
    ...
    def name(self) -> str:
        return self._master.name() + ' Predict Plugin'
    
    def master(self) -> str:
        return self._master.name()

    def displayName(self) -> str:
        return self._master.displayName() + '/Predict'
    ...
    def display(self) -> None:
        mi, mient, ma, maent = self._oracle.predict(self._modlist)
        QMessageBox.information(
            None,
            'Oracle',
            f'Estimated crash probability: {mi:.2%} - {ma:.2%}\nEntropies: {mient:.2f} - {maent:.2f}',
