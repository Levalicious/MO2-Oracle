...
class iRowBuilder(ABC, Generic[T]):
    """
    Interface for mapping nodes to columns.
    """

    @abstractmethod
    def map_node(self, nd: T) -> tuple[int, int]:
        """
...
class AbstractColumn(Generic[T]):
    ...
    def __init__(
        self, column: int, row: "AbstractRow[T]", color: int = 0, remaining: int = 0
    ):
        self.column = column
        self.start_row = row
        self.next_row = row
...
class AbstractRow(Generic[T]):
    ...
    _builder: iRowBuilder[T]
    row: int
    "The 0-based index of this row"

    columns: dict[int, AbstractColumn[T]]
    ...
    def __iter__(self) -> Iterator[AbstractColumn[T]]:
        for col in sorted(self.columns.values(), key=lambda c: c.column):
            yield col

    def __init__(
        self,
        builder: iRowBuilder[T],
        row: int,
        columns: Mapping[int, AbstractColumn[T]],
        label: str,
    ):
        """
        ...
        self.row = row
        self.columns = {
            k: AbstractColumn[T].copy_from_last(s)
            for k, s in columns.items()
            if s.remaining != 0
        }
        self.label = label

    def add_input(self, pred: T) -> None:
        """
    ...
    def add_node(self, nd: T, nsuccs: int) -> None:
        """
...
@dataclass
class AbstractPlot(iRowBuilder[T]):
    """
    ...
    rows: list["AbstractRow[T]"] = field(default_factory=list)
    "The rows of this visualization."

    _gaps: list[int] = field(default_factory=list)
    _mapping: dict[T, int] = field(default_factory=dict)

    def add_row(self, label: str) -> AbstractRow[T]:
        """
